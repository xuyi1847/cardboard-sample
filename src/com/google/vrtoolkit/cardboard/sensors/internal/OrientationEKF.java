...
// Referenced classes of package com.google.vrtoolkit.cardboard.sensors.internal:
//			Matrix3x3d, Vector3d, So3Util

public class OrientationEKF {

	private static final float NS2S = 1E-009F;
	...
	public double getHeadingDegrees() {
		...
		if (mag < 0.1D)
			return 0.0D;
		double heading = -90D - (Math.atan2(y, x) / 3.141592653589793D) * 180D;
		if (heading < 0.0D)
			heading += 360D;
		if (heading >= 360D)
			heading -= 360D;
	...
	public synchronized void setHeadingDegrees(double heading) {
		...
		Matrix3x3d.mult(so3SensorFromWorld, setHeadingDegreesTempM1,
				so3SensorFromWorld);
	}
	...
	public double[] getPredictedGLMatrix(double secondsAfterLastGyroEvent) {
		...
		pmu.set((double) lastGyro[0] * -dT, (double) lastGyro[1] * -dT,
				(double) lastGyro[2] * -dT);
		...
		Matrix3x3d.mult(so3PredictedMotion, so3SensorFromWorld,
				so3PredictedState);
	...
	public synchronized void processGyro(float gyro[], long sensorTimeStamp) {
		...
		if (sensorTimeStampGyro != 0L) {
			...
			Matrix3x3d.mult(so3LastMotion, so3SensorFromWorld,
					processGyroTempM1);
	...
	public synchronized void processAcc(float acc[], long sensorTimeStamp) {
		...
		if (sensorTimeStampAcc != 0L) {
			...
			Matrix3x3d.mult(processAccTempM4, processAccTempM3,
					processAccTempM5);
	...
	public void processMag(float mag[], long sensorTimeStamp) {
		...
		if (sensorTimeStampMag != 0L) {
			...
			for (int dof = 0; dof < 3; dof++) {
				...
				Matrix3x3d.mult(processMagTempM1, so3SensorFromWorld,
						processMagTempM2);
			...
			Matrix3x3d.mult(processMagTempM5, processMagTempM4,
					processMagTempM6);
			...
			So3Util.sO3FromMu(mx, so3LastMotion);
			Matrix3x3d
					.mult(so3LastMotion, so3SensorFromWorld, processMagTempM4);
	...
	private void filterGyroTimestep(float timeStep) {
		...
		if (!timestepFilterInit) {
			...
			filteredGyroTimestep = 0.95F * filteredGyroTimestep + 0.05000001F
					* timeStep;
	...
	private void updateCovariancesAfterMotion() {
		...
		Matrix3x3d.mult(mP, updateCovariancesAfterMotionTempM1,
				updateCovariancesAfterMotionTempM2);
	...
	private void accObservationFunctionForNumericalJacobian(
		...
		So3Util.sO3FromTwoVec(mh, mz,
				accObservationFunctionForNumericalJacobianTempM);
		So3Util.muFromSO3(accObservationFunctionForNumericalJacobianTempM,
				result);
	}
	...
	private void magObservationFunctionForNumericalJacobian(
		...
		So3Util.sO3FromTwoVec(mh, mz,
				magObservationFunctionForNumericalJacobianTempM);
		So3Util.muFromSO3(magObservationFunctionForNumericalJacobianTempM,
				result);
	}
